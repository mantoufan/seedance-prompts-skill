...
Claude 进入阶段 0：`/start` 确认题材/受众/基调/集数 → `/plan` 三幕结构 + 付费卡点 + 爽点矩阵 → `/characters` 角色 + 四层反派 → `/outline` 60 集目录（🔥关键集 / 💰卡点集）→ `/episode 1` 输出单集 △ 剧本 → `/review` 自检。剧本完成后可一键转入下游视频化。

**② 已有剧本/小说做视频（下游）**

> 把这段小说改成 Seedance 2.0 沉浸式短片分镜：{粘贴正文}

Claude 输出：完整剧本（△镜头 + 对白 + OS/VO）→ C01–C99 / S01–S99 / P01–P99 资产生成提示词（人物套用真实感关键词）→ 15s 时间轴分镜（音画一体、对白对齐、结尾帧续接），可直接投喂小云雀 / 即梦。
