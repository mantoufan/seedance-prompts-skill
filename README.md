...
> **可用平台**　国内：<a href="https://jimeng.jianying.com" target="_blank">即梦</a>、<a href="https://xyq.jianying.com" target="_blank">小云雀</a>、<a href="https://www.liblib.tv/" target="_blank">LibTV</a>；境外：<a href="https://dreamina.capcut.com" target="_blank">Dreamina（即梦国际版）</a>、<a href="https://www.capcut.com" target="_blank">CapCut</a>、<a href="https://cv.cm/v" target="_blank">CV.CM（免排队）</a>、<a href="https://higgsfield.ai/" target="_blank">Higgsfield</a>。
...
- **（上游·可选）一句创意/小说 → 完整短剧剧本**：从选题立项到分集撰写、质量自检、合规审核、海外出海，写出完整文学剧本（集数可自定义，常见 50-100 集；单集时长符合广电总局微短剧规范；渐进式命令流 `/start /plan /characters /outline /episode /review /compliance /export`）。
- **文章/故事 → 视频脚本**：将完整文本或简短大纲改编为标准脚本格式（△镜头描述 + 对白 + OS/VO + 闪回 + 字幕）。
- **生成 Seedance 2.0 分镜提示词**：按用户素材、目标时长与平台限制，自适应使用时间轴或镜头序列，音画一体、可直接复制使用。
- **多集系列规划**：按用户设定的集数、总时长和发布平台做分段/分集与连续性管理，不默认强拆成固定 15 秒或固定集数。
- **资产生成提示词**：为 Nana Banana Pro / GPT-image 等图像模型批量生成角色（C01–C99）、场景（S01–S99）、道具（P01–P99）提示词。

## 核心特性

- **12 条生产铁律 + 防污染原则**（SKILL.md 顶部，最高优先级）：先尊重用户任务，不把所有项目强套短剧 SOP；并覆盖时长分段计算、续接残影加固、无字幕规则、对白时间轴对齐、审核规避、TTS 配音避坑、动作可分性、双人手部锁定、输出排版、声线锁定、**视听语言优先**、**提示词即视觉语言 · 去 AI 感**。
