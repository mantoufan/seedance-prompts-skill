...
- **短剧剧本创作模块（上游）**（references/short-drama-screenwriting.md + references/short-drama/，改编自 MIT 开源 [0xsline/short-drama](https://github.com/0xsline/short-drama)）：13 种题材 + 出海文化映射、开篇黄金法则、付费卡点 5 套路、节奏四阶段波形、5 大爽点矩阵、四层反派体系、5 种钩子、合规红线清单——把一句创意写成完整剧本（集数可自定义，常见 50-100 集），再无缝接入下游视频化（同用 △ + 景别格式）。
